...
sub j2m
{
    ...
    $j2mt{$d} = [$k1, $mn, $article];
    return $j2mt{$d};
}

my %busted = (); 
@busted{ ("amistd_4", "amistd_5", "boole", "cc0sp1",
	  "cgames_1", "exchsort", "ltlaxio1", "matrtop1", "matrtop2", "ndiff_3",
	  "numerals", "ordinal6", "pdiff_8", "reloc", "subset", "tarski",
	  "taski") } = ();
...
while(<>)
{
  s/[\r\n]//;
  my @deps= split(/ +/);
  my ($k, $frst, $art) = @{j2m($deps[0])};
  if(($k =~ m/^(t|s|d|ie|[rcf]c)$/) && ! (exists $busted{$art}))
#  if($k =~ m/^t$/)
  {
      print ($frst , ' ');
